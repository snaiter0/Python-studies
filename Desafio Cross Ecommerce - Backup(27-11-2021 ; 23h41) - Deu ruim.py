...
page = 9850                                 #Inicia a variavel Pagina, que será a localização onde o programa irá
...
while (page != -1):                                          #Laço de repetição para consumir todas as paginas da API
    try:
        webUrl = urllib.request.urlopen(f'http://challenge.dienekes.com.br/api/numbers?page={page}')  #URL da API
                                                                # com a variavel "page", capaz de ser substituida, e
                                                                # assim avançar entre as paginas.
        ...
        if (len(espacoAposVirgula) <= 1):  # Confere se o array está vazio
            ...
            if (PaginasSemResposta != 0):
                response = input(
                    "Digite 1 para tentar recuperar as paginas sem retorno ou qualquer outra tecla para finalizar:")
                print("")
                if (response == '1' and PaginasSemResposta != 0):
                    mergeSort(espacoAposVirgula)  # Executa a função merge sort
                    page = EnderecoDasPaginasComErro.pop()
                    print("if2")  # consumida
                    print(f"Pagina atual: {page}")  # Exibe a pagina atual consumida
                    print("Array ordenado: ", end="\n")
                    for i in range(len(EnderecoDasPaginasComErro)):
                        print(espacoAposVirgula[i], end="")
                        PaginasSemResposta -= 1

                if (response == '1'):
                    print("Tentativa de recuperação iniciada.", end="\n")
                else:
                    page = -1  # Condição para sair do laço de repetição
                    break

        # Driver Code
        if (__name__ == '__main__' and response != '1'):
            print(f"Pagina atual: {page}")              #Exibe a pagina atual consumida
            print("Array ordenado: ", end="\n")
        ...
        print("Todas as paginas foram recuperadas.")
        break
